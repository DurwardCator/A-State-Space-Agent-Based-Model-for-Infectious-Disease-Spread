...
num_tests = 5000 # number of tests available per day
...
bkf_tp = jnp.zeros((num_ensemble,num_days))
bkf_fp = jnp.zeros((num_ensemble,num_days))
bkf_fn = jnp.zeros((num_ensemble,num_days))
...
for en in tqdm(range(num_ensemble),position=0,leave=True):
    ...
    for i in tqdm(range(num_days),position=1,leave=False):
        ...
        bkf_tp = bkf_tp.at[en,i].set(jnp.sum(jnp.logical_and(  Z_k[:,0],   agents[:,0])))
        bkf_fp = bkf_fp.at[en,i].set(jnp.sum(jnp.logical_and(  Z_k[:,0], 1-agents[:,0])))
        bkf_fn = bkf_fn.at[en,i].set(jnp.sum(jnp.logical_and(1-Z_k[:,0],   agents[:,0])))
...
print('Average BKF Precision = ', jnp.sum(bkf_tp) / (jnp.sum(bkf_tp) + jnp.sum(bkf_fp)))
print('Average BKF Recall = ', jnp.sum(bkf_tp) / (jnp.sum(bkf_tp) + jnp.sum(bkf_fn)))
